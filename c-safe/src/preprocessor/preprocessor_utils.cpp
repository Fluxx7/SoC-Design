...
int macro_scan(array_vector<macro_def>& macros, array_vector<f_string>& constants, array_vector<int>& const_values, array_vector<static_def>& statics, FILE* file, const char* path) {
    ...
    while(1) {
        ...
        if (rawline.smatch("INCLUDE ")) {
            ...
            // build the path
            f_string path_copy = path;
            path_copy += '\0';
            newfile_relative = dirname(path_copy);
            newfile_relative += "/";
            newfile_relative += newfile;

            included = fopen(newfile_relative, "r");
            if (!included) {
                return compile_error("failed to open file '{}'", newfile);
            }
            if (macro_scan(macros, constants, const_values, statics, included, path) == -1) return -1;
            fclose(included);
